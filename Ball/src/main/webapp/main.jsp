<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" session="true" %>
    <%@ page import="dto.User, dao.MatchDAO, dto.Match, java.util.*, java.time.*" %>
        ...
        <% // 로그인 체크 Object obj=session.getAttribute("loginUser"); if (obj==null) {
            response.sendRedirect(request.getContextPath() + "/index.jsp" ); return; } dto.User loginUser=(dto.User)
            obj; // 관리자면 관리자 페이지 이동 if ("admin".equalsIgnoreCase(loginUser.getRole())) {
            response.sendRedirect(request.getContextPath() + "/admin/admin_main.jsp" ); return; } String
            displayName=(loginUser.getName() !=null) ? loginUser.getName() : String.valueOf(obj); String
            ctx=request.getContextPath(); LocalDate today=LocalDate.now(); MatchDAO mdao=new MatchDAO(); List<Match>
            matches = mdao.getTodayMatches();
            %>
                    ...
                    <div class="container px-4 px-lg-5">
                        ...
                        <div class="hero-box row align-items-center shadow">
                            ...
                            <div class="col-lg-6 text-center">
                                <img src="<%=ctx%>/assets/img/football.png" class="img-fluid rounded"
                        ...
                        <div class="list-group shadow-sm mb-5">
                            ...
                            <% if (matches==null || matches.isEmpty()) { %>
                                ...
                                <% } else { for (Match m : matches) { String timeStr=m.getMatchTime() !=null ?
                                    m.getMatchTime().toString() : "시간 없음" ; String place=m.getLocation(); int
                                    cur=m.getCurrentPlayers(); int max=m.getMaxPlayers(); String
                                    status=m.getMatchStatus(); String badgeText="" ; String badgeColor="" ; if
                                    ("취소됨".equals(status)) { badgeText="경기 취소" ; badgeColor="danger" ; } else if (cur>=
                                    max) {
                                    badgeText = "예약 마감";
