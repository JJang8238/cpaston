<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" session="true" %>
    <%@ page import="dao.MatchDAO, dao.PlaceReviewDAO, dto.Match, dto.User, java.util.*" %>
        ...
        <% // 로그인 체크 Object loginObj=session.getAttribute("loginUser"); if (loginObj==null) {
            response.sendRedirect("index.jsp"); return; } User loginUser=(User) loginObj; int
            loginUserId=loginUser.getId(); String ctx=request.getContextPath(); // 요청한 장소 String
            place=request.getParameter("place"); if (place==null) place="" ; // DAO MatchDAO matchDAO=new MatchDAO();
            PlaceReviewDAO reviewDAO=new PlaceReviewDAO(); // 리뷰 목록 List<Map<String, Object>> reviewList = null;
            double avg = 0;
            int count = 0;
                        ...
                        <div class="container px-4 px-lg-5 my-4">
                                ...
                                <div class="col-md-6">
                                    <select name="place" class="form-select">
                                        ...
                                        <% for (String p : allPlaces) { %>
                                            <option value="<%=p%>" <%=p.equals(place) ? "selected" : "" %>><%=p%>
                                            </option>
                                            <% } %>
                                    </select>
                                </div>
                            ...
                            <% if (!place.isBlank()) { %>
                                ...
                                <% } %>
                                    ...
                                    <div class="mb-4">
                                        <% if (place.isBlank()) { %>
                                            ...
                                            <% } else if (reviewList==null || reviewList.isEmpty()) { %>
                                                ...
                                                <% } else { for (var r : reviewList) { int
                                                    rating=Integer.parseInt(String.valueOf(r.get("rating"))); String
                                                    stars="★" .repeat(rating); int
                                                    writerId=Integer.parseInt(String.valueOf(r.get("userId"))); %>
                                                            ...
                                                            <% if (writerId==loginUserId) { %>
                                                                <div class="mt-2">
                                                                    <button class="btn btn-sm btn-outline-primary"
                                                                        onclick="openEditModal(<%=r.get(" id")%>, '
                                                                        <%=r.get("content")%>', <%=rating%>)">
                                                                                수정
                                                                    </button>
                                    ...
                                    <% if (!place.isBlank()) { %>
                                        <% if (canWrite) { %>
                                            <button class="btn btn-success w-100 py-2" onclick="openWriteModal()">리뷰
                                                작성하기</button>
                                            <% } else { %>
                                                <button class="btn btn-secondary w-100 py-2" disabled>⚠ 예약한 경기장만 리뷰 작성이
                                                    가능합니다</button>
                                                <% } %>
                                                    <% } %>
                                ...
                                <div class="modal-content">
                                    <form onsubmit="return submitReview();">
                                        ...
                                        <div class="modal-body">
                                            <textarea id="reviewContent" class="form-control mb-3" rows="4"
                                ...
                                <div class="modal-content">
                                    <form onsubmit="return updateReview();">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <textarea id="editReviewContent" class="form-control mb-3"
                                                rows="4"></textarea>
